...
puts "density_counterions $density_counterions"
...
electrokinetics agrid 1 lb_density $density_solution viscosity $viscosity friction 1.0 T 1 bjerrum_length 0.7095 use_nonlinear_stencil $use_nonlinear_stencil

puts "density_pos=[expr $density_salt+$density_counterions]"
...
electrokinetics 1 density [expr $density_salt+$density_counterions] D $D_pos valency 1.0 ext_force [expr $ext_force] 0.0 0.0
...
for {set i 1} {1} {incr i} {
  ...
  if {[catch {exec grep -i -q nan "$output_folder/density_pos.vtk" "$output_folder/density_neg.vtk" "$output_folder/density_neutral.vtk" "$output_folder/velocity.vtk"}] != 1} {
